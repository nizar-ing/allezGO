...
const HOTELS_DATA = [
    {
        ...
        cities: [
            ...
            "Nabeul", "Mahdia", "Gammarth", "Tozeur", "Bizerte",
        ],
    },
    {
        name: "Algérie",
        ...
        cities: [
            ...
            "Boumerdes", "Batna", "Eloued",
        ],
    },
];

/**
 * HotelsPopup Component - Enhanced with Tab Navigation
 */
const HotelsPopup = ({
                         onClose,
                         onCitySelect,
                         isVisible,
                         countries = HOTELS_DATA,
                         className = "",
                         style = {}
                     }) => {
    const popupRef = useRef(null);
    const [activeTab, setActiveTab] = useState(0);
    const totalCities = countries.reduce((acc, country) => acc + country.cities.length, 0);

    const activeCountry = countries[activeTab];
        ...
        >
                ...
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <div>
                        ...
                        <p className="text-[10px] sm:text-xs text-gray-600 font-mono mt-0.5 sm:mt-1">
                            {totalCities} destinations disponibles
                        </p>
                    </div>
                    <button
                        onClick={onClose}
            ...
            >
                <div className="hotels-fade-in">
                    {/* Country Header with enhanced styling */}
                    <div className="flex items-center gap-2.5 mb-4 pb-3 border-b-2 border-gradient">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-600 mt-0.5b font-mono">
                                Selectionnez votre preferable city
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="hotels-grid gap-2 sm:gap-3">
                        {activeCountry.cities.map((city, cityIndex) => (
                            <button
                                key={city}
                                onClick={() => {
                                    onCitySelect(city);
                                ...
                                className="group/btn relative px-3 sm:px-4 py-3 sm:py-3.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-orange-500 hover:text-white hover:border-orange-500 hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 text-left overflow-hidden"
                                style={{
                            ...
                            >
                                <div className="relative flex items-center justify-between gap-2">
                                    <span className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                                        <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 group-hover/btn:text-white transition-colors flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                        </svg>
                                        <span className="truncate font-semibold">{city}</span>
                                    </span>
                                    <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-300 group-hover/btn:text-white group-hover/btn:translate-x-0.5 transition-all flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="sticky bottom-0 bg-gradient-to-br from-gray-50 to-white rounded-b-2xl border-t border-gray-100 px-3 sm:px-5 py-2 sm:py-3">
                <p className="text-xs font-mono text-gray-500 text-center">
                    <span className="font-semibold text-orange-600">{activeCountry.cities.length} cities</span> disponibles en {activeCountry.name}
                </p>
            </div>
