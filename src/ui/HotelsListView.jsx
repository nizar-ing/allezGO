...
import {
    X, Search, Hotel, AlertCircle,
    Filter, ArrowUpDown, ChevronDown, CheckCircle,
} from 'lucide-react';
...
function HotelsListView({
    ...
    // ── Derive nights from searchCriteria ──────────────────────────────────────
    const nights = useMemo(() => {
        if (searchCriteria?.nights) return searchCriteria.nights;
        if (!searchCriteria?.checkIn || !searchCriteria?.checkOut) return 1;
        return Math.ceil(
            Math.abs(
                new Date(searchCriteria.checkOut) - new Date(searchCriteria.checkIn)
            ) / (1000 * 60 * 60 * 24)
        );
    }, [searchCriteria]);

    // ── Pricing resolver ───────────────────────────────────────────────────────
    // ✅ Path 1: hotel.pricing already embedded by parent (HotelsPerCityPage /
    //            HotelsSearchResultsPage) → zero extra work.
    // ✅ Path 2: fallback — compute minPrice from raw Price tree via
    //            getTarifForHotel() for backward-compatible callers.
    ...
    const favoriteIds = useMemo(
        ...
        [] // intentionally empty — favorites are managed per-card, not re-derived here
    );
    ...
    const filteredHotels = useMemo(() => {
        ...
        if (filters.categories?.length > 0)
            result = result.filter(h =>
                filters.categories.includes(h.Category?.Star)
            );
        if (filters.services?.length > 0)
            result = result.filter(h =>
                h.Theme?.some(theme =>
                    filters.services.some(s =>
                        theme.toLowerCase().includes(s.toLowerCase())
                    )
                )
            );
        return result;
    ...
    // ── Sort ────────────────────────────────────────────────────────────────────
    // ✅ Fixed: uses getPricingForHotel(h).minPrice instead of nonexistent TotalPrice
    const sortedHotels = useMemo(() => {
        const sorted = [...filteredHotels];
        switch (sortBy) {
            case 'price-asc':
                return sorted.sort((a, b) =>
                    (getPricingForHotel(a)?.minPrice ?? Infinity) -
                    (getPricingForHotel(b)?.minPrice ?? Infinity)
                );
            case 'price-desc':
                return sorted.sort((a, b) =>
                    (getPricingForHotel(b)?.minPrice ?? 0) -
                    (getPricingForHotel(a)?.minPrice ?? 0)
                );
            case 'rating':
                return sorted.sort((a, b) =>
    ...
    // ── Reset display count on filter/sort change ──────────────────────────────
    useEffect(() => {
    ...
    const loadMore        = useCallback(() =>
        setDisplayCount(prev => prev + HOTELS_PER_PAGE), []);
    ...
    const handleFavoriteToggle = useCallback((hotelId, isFav) => {
        ...
        if (isFav) {
            if (!favorites.includes(hotelId)) favorites.push(hotelId);
        } else {
            const i = favorites.indexOf(hotelId);
                                ...
                                {displayedHotels.map(hotel => {
                                    ...
                                    return (
                                        <HotelLightCard
                                            key={hotel.Id}
                                            hotel={hotel}
                                            // ✅ Fixed: was "tarif" (wrong/ignored) → now "pricing"
                                            pricing={pricing}
                                            // ✅ NEW: instant Tarifs panel — no second API call
                                            preloadedAvailability={hotel._preloadedRooms ?? null}
                                            onFavoriteToggle={handleFavoriteToggle}
                                            // ✅ Fixed: was always missing → card shows Réserver button
                                            showBookButton={showPricing}
                                            // ✅ Fixed: was always defaulting to 1 inside card
                                            nights={nights}
                                            // ✅ searchCriteria carries checkIn/checkOut/rooms
                                            searchParams={searchCriteria}
                                            // ✅ Fixed: was always false (heart state wrong)
                        ...
                        {hasNextPage && (
                            <div ref={loadMoreRef} className="mt-6 sm:mt-8">
                                <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg flex justify-center p-4">
                                    <button
                                        onClick={loadMore}
                                        className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-sky-600 hover:bg-sky-700 text-white font-bold text-base sm:text-lg rounded-xl transition-all shadow-lg hover:shadow-xl flex items-center justify-center gap-2 active:scale-95"
                                    >
